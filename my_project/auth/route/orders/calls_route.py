...
@calls_bp.get("/caller/<int:caller_id>")
def get_callers(caller_id: int) -> Response:
    return make_response(jsonify(calls_controller.find_callers(caller_id)), HTTPStatus.OK)

@calls_bp.get("/city/<int:city_id>")
def get_cities(city_id: int) -> Response:
    return make_response(jsonify(calls_controller.find_cities(city_id)), HTTPStatus.OK)
